...
group_avgs %>% 
  ...
  labs(y = "Proportion of probable response",
       x = "Block (20 trials)",
       fill = "",
       colour = "")
...
# load in the PKH82 results
load("Simulations NP learning/UNLL01_PKH82_results.RData")
PKH82_raw <- vs_res 
PKH82_raw <- PKH82_raw %>% mutate(model = "PKH82", .before = 1) %>% ungroup()

complete_sims <- rbind(RW72_raw, Mack75_raw, PKH82_raw)
...
# plot Mack75 raw sim results as line graph for both associations
PKH82_line_plot <- 
  filter(complete_sims, model == "PKH82") %>% 
    make_model_line_plot(title = "PKH82 line plot all stages")
...
emp_data_fig <- 
  p_pred_avgs %>%
  filter(condition_rank <= 20) %>% 
  mutate(stage = case_match(stage, "stage1" ~ "Stage 1", "stage2" ~ "Stage 2", "stage3" ~ "Stage 3")) %>% 
  ...
  geom_col(position = position_dodge(.9), linewidth = 1, colour = "black") +
  geom_errorbar(aes(ymin = grp_avg - grp_se,
                    ymax = grp_avg + grp_se),
                position = position_dodge(.9),
                width = .2) +
  labs(y = "Rating difference score",
       x = element_blank(),
       fill = element_blank())+
  ...
  theme(axis.text.x = element_text(size = 30),
        legend.text = element_text(size = 30)) +
  scale_fill_manual(values = c("#404040", "#D3D3D3"))

# summarise and plot empirical data for critical stage 2 comparisons (for SEPC talk)
emp_data_fig_critical <- 
  p_pred_avgs %>%
  filter(condition_rank <= 20,
         stage %in% c("stage2", "stage3"),
         condition %in% c("C-U", "U-U")) %>% 
  mutate(stage = case_match(stage, "stage2" ~ "Stage 2", "stage3" ~ "Stage 3")) %>% 
  group_by(condition, stage, cue_type) %>% 
  summarise(grp_avg = mean(avg_diff_score),
            grp_se = sd(avg_diff_score)/sqrt(n())) %>% 
  ggplot(aes(x = condition, y = grp_avg, fill = cue_type)) +
...
PKH82_fig <- 
  sim_data_cue_types %>% 
  filter(model == "PKH82") %>% 
  make_model_col_plot(titleIn = "PKH82 simulation")

emp_data_fig / mack75_fig / RW72_fig / PKH82_fig
```
